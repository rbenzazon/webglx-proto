...
gl.shaderSource(fragmentShader, `
    ...
    void main() {
        ...
        offset = lightPosition2 - vertex;
        distance = length(offset);
        direction = normalize(offset);

        diffuse = max(dot(direction, vNormal), 0.0);
        attenuation = 3.0 / (1.0 + 0.1*distance + 0.1*distance*distance);
        brightness += max(diffuse * attenuation,0.1);
...
const lightPosition2Location = gl.getUniformLocation(program, "lightPosition2")
let lightPosition2 = [0, 2, -1];
gl.uniform3fv(lightPosition2Location, lightPosition2);
...
modelMatrices.forEach((mat, index) => {
    ...
    const count = index - Math.floor(numInstances / 2) - row *5 ;
    identity(mat);
    translate(mat,mat,[count * 2, row *2 , 0]);
...
function transformInstances(){
    ...
    modelMatrices.forEach((mat, index) => {
        const instanceShift = index*0.0001*Math.PI;
...
function loop(){
    ...
    lightPosition = [...getCirclePoint([0,0],3,(performance.now() / 1000) * Math.PI * 0.5), -2];
    gl.uniform3fv(lightPositionLocation, lightPosition);
    const circle = getCirclePoint([0,0],5,(performance.now() / 1000) * -Math.PI * 0.4);
    lightPosition2 = [circle[0], 2,circle[1]];
    gl.uniform3fv(lightPosition2Location, lightPosition2);
    /*normalMatrix = create();
    invert(normalMatrix, view);
    transpose(normalMatrix, normalMatrix);
